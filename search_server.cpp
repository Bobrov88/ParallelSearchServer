...
void SearchServer::AddDocument(int document_id, const std::string& document, DocumentStatus status, const std::vector<int>& ratings) {
    ...
    std::tuple<std::vector<std::string>, DocumentStatus> SearchServer::MatchDocument(std::execution::parallel_policy p, const std::string& raw_query, int document_id) const {
    ...
    std::tuple<std::vector<std::string>, DocumentStatus> SearchServer::MatchDocument(std::execution::sequenced_policy p, const std::string& raw_query, int document_id) const {
        if (documents_.count(document_id) == 0) throw std::out_of_range("No such document");
    ...
    SearchServer::Query SearchServer::ParseQuery(std::execution::parallel_policy p, const std::string& text) const {
        SearchServer::Query result;
        std::vector<std::string> split = SplitIntoWords(text);
        std::vector<std::string> pluses(split.size());
        std::vector<std::string> minuses(split.size());
        for_each(p, split.begin(), split.end(), [&pluses, &minuses, this](const auto& word) {
            const auto query_word = ParseQueryWord(word);
            if (!query_word.is_stop) {
                if (query_word.is_minus) {
                    minuses.push_back(query_word.data);
                }
                else {
                    pluses.push_back(query_word.data);
                }
            }
            });
        std::sort(p, minuses.begin(), minuses.end());
        auto it_minuses = std::unique(p, minuses.begin(), minuses.end());
        std::sort(p, pluses.begin(), pluses.end());
        auto it_pluses = std::unique(p, pluses.begin(), pluses.end());
        pluses.resize(std::distance(pluses.begin(), it_pluses));
        minuses.resize(std::distance(minuses.begin(), it_minuses));
        result.plus_words = { pluses.begin(), pluses.end() };
        result.minus_words = { minuses.begin(), minuses.end() };
...
std::map<string, double> SearchServer::GetWordFrequencies(int document_id) const {
        if (word_frequencies_.count(document_id))
            return word_frequencies_.at(document_id);
        return {};
}

std::map<int, std::set<string>>& SearchServer::getAccessToSetOfWords() {
...
/*        auto doc_erase = std::move(word_frequencies_[document_id]);
   ...
   std::vector<std::pair<const std::string, double>*> v(word_frequencies_[document_id].size());
    std::transform(p,
